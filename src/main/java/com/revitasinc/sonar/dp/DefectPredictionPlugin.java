 ...
 */
@Properties({
    @Property(key = DefectPredictionPlugin.COMMENT_REGEX, name = "Comment Regex", description = "A regular expression (Java style) for including revisions by commit comment"),
    @Property(key = DefectPredictionPlugin.FILE_NAME_REGEX, name = "File Name Regex", description = "A regular expression (Java style) for file names to include"),
    @Property(key = DefectPredictionPlugin.COMMAND, name = "SCM Command", description = "The command to run on this system to generate a log file from your source code management system"),
    @Property(key = DefectPredictionPlugin.SOURCE_PATH, name = "Source Path", description = "Path to the root folder of the source code, either absolute or relative to the project path"),
